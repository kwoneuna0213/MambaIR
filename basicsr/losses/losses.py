...
@LOSS_REGISTRY.register()
class BackgroundConsistencyLoss(nn.Module):
    """Multi-scale Gaussian blur L1 loss for background consistency.

    Args:
        sigmas (tuple[float] | list[float]): Gaussian sigma values.
        lambdas (tuple[float] | list[float]): Per-scale weights.
        loss_weight (float): Global loss weight. Default: 1.0.
    """

    def __init__(self, sigmas=(5, 9, 15), lambdas=(0.25, 0.5, 1.0), loss_weight=1.0):
        super(BackgroundConsistencyLoss, self).__init__()
        if len(sigmas) != len(lambdas):
            raise ValueError('sigmas and lambdas must have the same length.')
        ...
        for sigma in self.sigmas:
            ...
            self.register_buffer(f'kernel_{self._sigma_key(sigma)}', kernel)
    ...
    def _blur(self, x, sigma):
        key = self._sigma_key(sigma)
        kernel = getattr(self, f'kernel_{key}')
        c = x.shape[1]
    ...
    def forward(self, input_img, output_img):
        loss = output_img.new_tensor(0.0)
        ref = input_img.detach()
        for sigma, lam in zip(self.sigmas, self.lambdas):
            b_in = self._blur(ref, sigma)
            b_out = self._blur(output_img, sigma)
            loss = loss + lam * F.l1_loss(b_in, b_out)
